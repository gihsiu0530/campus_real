#!/usr/bin/env python3
"""Pure IMU dead reckoning using OpenVINS-style RK4 mean propagation."""
...
def quat_multiply_jpl(q, p):
    qv = q[:3]
    qm = np.zeros((4, 4))
    qm[:3, :3] = q[3] * np.eye(3) - skew(qv)
    qm[:3, 3] = qv
    qm[3, :3] = -qv
    qm[3, 3] = q[3]
    return quatnorm_jpl(qm @ p)


def quat_to_rot_jpl(q):
    q = quatnorm_jpl(q)
    qv = q[:3]
    qx = skew(qv)
    return (2.0 * q[3] * q[3] - 1.0) * np.eye(3) - 2.0 * q[3] * qx + 2.0 * np.outer(qv, qv)


def rot_to_quat_jpl(rot):
    rot = np.asarray(rot, dtype=float)
    tr = np.trace(rot)
    q = np.zeros(4)
    if rot[0, 0] >= tr and rot[0, 0] >= rot[1, 1] and rot[0, 0] >= rot[2, 2]:
        q[0] = math.sqrt(max(0.0, (1.0 + 2.0 * rot[0, 0] - tr) / 4.0))
        q[1] = (rot[0, 1] + rot[1, 0]) / (4.0 * q[0])
        q[2] = (rot[0, 2] + rot[2, 0]) / (4.0 * q[0])
        q[3] = (rot[1, 2] - rot[2, 1]) / (4.0 * q[0])
    elif rot[1, 1] >= tr and rot[1, 1] >= rot[0, 0] and rot[1, 1] >= rot[2, 2]:
        q[1] = math.sqrt(max(0.0, (1.0 + 2.0 * rot[1, 1] - tr) / 4.0))
        q[0] = (rot[0, 1] + rot[1, 0]) / (4.0 * q[1])
        q[2] = (rot[1, 2] + rot[2, 1]) / (4.0 * q[1])
        q[3] = (rot[2, 0] - rot[0, 2]) / (4.0 * q[1])
    elif rot[2, 2] >= tr and rot[2, 2] >= rot[0, 0] and rot[2, 2] >= rot[1, 1]:
        q[2] = math.sqrt(max(0.0, (1.0 + 2.0 * rot[2, 2] - tr) / 4.0))
...
def read_imu_bag(bag_path, topic):
    ...
    if not times:
        raise RuntimeError(f"no IMU messages found for topic {topic}")
...
def read_zero_start_odom(bag_path, topic):
    ...
    with rosbag.Bag(str(bag_path), "r") as bag:
        for _, msg, bag_time in bag.read_messages(topics=[topic]):
            ...
            quats.append(q)
    if not times:
        raise RuntimeError(f"no GT messages found for topic {topic}")
    order = np.argsort(np.asarray(times))
...
def predict_mean_rk4(q_gtoi, p_iing, v_iing, w_hat1, a_hat1, w_hat2, a_hat2, dt, gravity_vec):
    w_hat = np.array(w_hat1, dtype=float)
    a_hat = np.array(a_hat1, dtype=float)
    w_alpha = (w_hat2 - w_hat1) / dt
    a_jerk = (a_hat2 - a_hat1) / dt

    dq_0 = np.array([0.0, 0.0, 0.0, 1.0])
    q0_dot = 0.5 * omega_jpl(w_hat) @ dq_0
    p0_dot = v_iing
    r_gto0 = quat_to_rot_jpl(quat_multiply_jpl(dq_0, q_gtoi))
    v0_dot = r_gto0.T @ a_hat - gravity_vec
    k1_q = q0_dot * dt
    k1_p = p0_dot * dt
    k1_v = v0_dot * dt
    ...
    a_hat += 0.5 * a_jerk * dt
    dq_1 = quatnorm_jpl(dq_0 + 0.5 * k1_q)
    v_1 = v_iing + 0.5 * k1_v
    q1_dot = 0.5 * omega_jpl(w_hat) @ dq_1
    p1_dot = v_1
    r_gto1 = quat_to_rot_jpl(quat_multiply_jpl(dq_1, q_gtoi))
    v1_dot = r_gto1.T @ a_hat - gravity_vec
    k2_q = q1_dot * dt
    k2_p = p1_dot * dt
    k2_v = v1_dot * dt

    dq_2 = quatnorm_jpl(dq_0 + 0.5 * k2_q)
    v_2 = v_iing + 0.5 * k2_v
    q2_dot = 0.5 * omega_jpl(w_hat) @ dq_2
    p2_dot = v_2
    r_gto2 = quat_to_rot_jpl(quat_multiply_jpl(dq_2, q_gtoi))
    v2_dot = r_gto2.T @ a_hat - gravity_vec
    k3_q = q2_dot * dt
    k3_p = p2_dot * dt
    k3_v = v2_dot * dt
    ...
    w_hat += 0.5 * w_alpha * dt
    a_hat += 0.5 * a_jerk * dt
    dq_3 = quatnorm_jpl(dq_0 + k3_q)
    v_3 = v_iing + k3_v
    q3_dot = 0.5 * omega_jpl(w_hat) @ dq_3
    p3_dot = v_3
    r_gto3 = quat_to_rot_jpl(quat_multiply_jpl(dq_3, q_gtoi))
    v3_dot = r_gto3.T @ a_hat - gravity_vec
    k4_q = q3_dot * dt
    k4_p = p3_dot * dt
    k4_v = v3_dot * dt

    dq = quatnorm_jpl(dq_0 + k1_q / 6.0 + k2_q / 3.0 + k3_q / 3.0 + k4_q / 6.0)
    return (
        quat_multiply_jpl(dq, q_gtoi),
        p_iing + k1_p / 6.0 + k2_p / 3.0 + k3_p / 3.0 + k4_p / 6.0,
        v_iing + k1_v / 6.0 + k2_v / 3.0 + k3_v / 3.0 + k4_v / 6.0,
    )


def integrate_imu_rk4(times, gyros, accels, static_duration, gravity, accel_bias_mode):
    t0 = times[0]
    static_mask = times <= t0 + static_duration
    if np.count_nonzero(static_mask) < 3:
        raise RuntimeError("not enough IMU samples in static window")
    ...
    r_itog = rot_from_two_vectors(accel_init, np.array([0.0, 0.0, gravity]))
    q_gtoi = rot_to_quat_jpl(r_itog.T)
    ...
    for i in range(1, n):
        ...
        if dt <= 0.0 or dt > 1.0:
            ...
            vs[i] = vs[i - 1]
            continue
        w1 = gyros[i - 1] - gyro_bias
        w2 = gyros[i] - gyro_bias
        a1 = accels[i - 1] - accel_bias
        a2 = accels[i] - accel_bias
        qs[i], ps[i], vs[i] = predict_mean_rk4(qs[i - 1], ps[i - 1], vs[i - 1], w1, a1, w2, a2, dt, gravity_vec)
        a_globals[i] = quat_to_rot_jpl(qs[i]).T @ a2 - gravity_vec

    return ps, vs, qs, a_globals, gyro_bias, accel_bias
...
def save_imu_csv(path, times, positions, velocities, quats, lin_accels, gyro_bias, accel_bias):
    path = Path(path)
...
def save_xy_plot(path, imu_p, gt_p=None, title="OpenVINS RK4 IMU Dead Reckoning"):
    ...
    ax.plot(imu_p[:, 0], imu_p[:, 1], label="IMU RK4 dead reckoning", linewidth=2.0)
...
def main():
    parser = argparse.ArgumentParser(description="OpenVINS-style RK4 pure IMU dead reckoning from a rosbag.")
    ...
    parser.add_argument("--output", default="/home/cyc/open_vins/imu_dead_reckoning_rk4.csv", help="output IMU-only trajectory CSV")
    parser.add_argument("--error-csv", default="/home/cyc/open_vins/imu_odom_errors_rk4.csv", help="output error CSV when --odom-topic is set")
    parser.add_argument("--xy-plot", default="/home/cyc/open_vins/imu_gt_xy_overlay_rk4.png", help="output XY plot")
    parser.add_argument("--static-duration", type=float, default=5.0, help="initial stationary window for bias/gravity init [s]")
    ...
    positions, velocities, quats, lin_accels, gyro_bias, accel_bias = integrate_imu_rk4(
        times, gyros, accels, args.static_duration, args.gravity, args.accel_bias_mode
    )
    save_imu_csv(args.output, times, positions, velocities, quats, lin_accels, gyro_bias, accel_bias)
    ...
    if args.odom_topic:
        ...
        print_summary("IMU RK4 XY position", xy_err, "m")
        print_summary("IMU RK4 3D position", pos_err, "m")
        print(f"saved error CSV: {Path(args.error_csv).resolve()}")
    else:
        save_xy_plot(args.xy_plot, positions)
