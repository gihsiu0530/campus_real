...
def apply_position_offset(positions, quats, offset, frame):
    if frame == "global":
        return positions + offset
    out = []
    for p, q in zip(positions, quats):
        # OpenVINS CSV quaternions are stored as Hamilton q_ItoG in xyzw here,
        # so this maps a body/IMU-frame lever arm into the global frame.
        out.append(p + quat_xyzw_to_rot(q) @ offset)
...
def main():
    ...
    parser.add_argument(
        "--vio-imu-to-lidar-offset",
        action="store_true",
        help="compare the VIO IMU pose shifted to the lidar/GT origin before alignment",
    )
    parser.add_argument(
        "--vio-offset",
        type=parse_vec3,
        default=np.array([1.4, 0.0, -0.35], dtype=float),
        help="VIO position offset as x,y,z meters; default is 1.4,0,-0.35",
    )
    parser.add_argument(
        "--vio-offset-frame",
        choices=["body", "global"],
        default="body",
        help="frame of --vio-offset; body applies R_ItoG*offset at each pose, global adds a constant vector",
    )
    ...
    est_p_raw_m = est_p[est_idx]
    est_q_m = est_q[est_idx]
    est_p_m = est_p_raw_m
    if args.vio_imu_to_lidar_offset:
        est_p_m = apply_position_offset(est_p_raw_m, est_q_m, args.vio_offset, args.vio_offset_frame)
    gt_p_m = gt_p[gt_idx]
    gt_q_m = gt_q[gt_idx]
    ...
    if args.vio_imu_to_lidar_offset:
        print(
            "VIO offset before alignment: "
            f"{args.vio_offset[0]:.6f}, {args.vio_offset[1]:.6f}, {args.vio_offset[2]:.6f} m in {args.vio_offset_frame} frame"
        )
