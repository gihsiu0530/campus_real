 ...
 * If the requested output filename ends in .csv, output is comma delimited with a
 * EuRoC-compatible first eight columns:
...
public:
  ...
  Recorder(std::string filename) {
    output_csv = (boost::filesystem::path(filename).extension() == ".csv");
    ...
    if (output_csv) {
      outfile << "timestamp(ns),tx,ty,tz,qw,qx,qy,qz,Pr11,Pr12,Pr13,Pr22,Pr23,Pr33,Pt11,Pt12,Pt13,Pt22,Pt23,Pt33" << std::endl;
    } else {
...
protected:
  ...
  void write() {
    ...
    if (output_csv) {
      ...
      outfile << timestamp * 1e9;

      outfile.precision(9);
      outfile << "," << p_IinG.x() << "," << p_IinG.y() << "," << p_IinG.z() << "," << q_ItoG(3) << "," << q_ItoG(0) << ","
              << q_ItoG(1) << "," << q_ItoG(2);

      if (has_covariance) {
        outfile.precision(10);
        outfile << "," << cov_rot(0, 0) << "," << cov_rot(0, 1) << "," << cov_rot(0, 2) << "," << cov_rot(1, 1) << ","
                << cov_rot(1, 2) << "," << cov_rot(2, 2) << "," << cov_pos(0, 0) << "," << cov_pos(0, 1) << "," << cov_pos(0, 2)
                << "," << cov_pos(1, 1) << "," << cov_pos(1, 2) << "," << cov_pos(2, 2);
      }
      outfile << std::endl;
      return;
    }
