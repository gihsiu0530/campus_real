...
find_package(catkin QUIET COMPONENTS roscpp cv_bridge ov_core)

# Some catkin environments do not populate CMAKE_PREFIX_PATH while configuring a
# freshly initialized workspace. Fall back to the sibling ov_core package so the
# ROS1 build still sees the headers and library it depends on.
set(OV_CORE_LOCAL_INCLUDE "${CMAKE_CURRENT_SOURCE_DIR}/../ov_core/src")
if (EXISTS "${OV_CORE_LOCAL_INCLUDE}/cam/CamEqui.h")
    list(APPEND OV_INIT_EXTRA_INCLUDE_DIRS "${OV_CORE_LOCAL_INCLUDE}")
endif ()

set(OV_INIT_EXTRA_LIBRARIES "")
if (catkin_DIR)
    get_filename_component(OV_INIT_ROS_PREFIX "${catkin_DIR}/../../.." ABSOLUTE)
    if (EXISTS "${OV_INIT_ROS_PREFIX}/include/ros/ros.h")
        list(APPEND OV_INIT_EXTRA_INCLUDE_DIRS "${OV_INIT_ROS_PREFIX}/include")
    endif ()
    foreach (ROS_LIBRARY roscpp rosconsole rosconsole_log4cxx rosconsole_backend_interface roscpp_serialization xmlrpcpp rostime cpp_common)
        if (EXISTS "${OV_INIT_ROS_PREFIX}/lib/lib${ROS_LIBRARY}.so")
            list(APPEND OV_INIT_EXTRA_LIBRARIES "${OV_INIT_ROS_PREFIX}/lib/lib${ROS_LIBRARY}.so")
        endif ()
    endforeach ()
endif ()

set(OV_CORE_LOCAL_LIBRARY "${CMAKE_CURRENT_SOURCE_DIR}/../../../devel/lib/libov_core_lib.so")
if (EXISTS "${OV_CORE_LOCAL_LIBRARY}")
    list(APPEND OV_INIT_EXTRA_LIBRARIES "${OV_CORE_LOCAL_LIBRARY}")
endif ()
