...
#include "update/UpdaterVehicleDynamics.h"
...
VioManager::VioManager(VioManagerOptions &params_) : thread_init_running(false), thread_init_success(false) {
  ...
  // If we are using vehicle dynamics updates, then create the updater
  if (params.try_vehicle_dynamics) {
    Eigen::Matrix<double, 4, 1> q_imu_to_vehicle;
    q_imu_to_vehicle << params.vehicle_q_imu_to_vehicle.at(0), params.vehicle_q_imu_to_vehicle.at(1),
        params.vehicle_q_imu_to_vehicle.at(2), params.vehicle_q_imu_to_vehicle.at(3);
    updaterVehicleDynamics = std::make_shared<UpdaterVehicleDynamics>(
        params.imu_noises, q_imu_to_vehicle, params.vehicle_wheelbase, params.vehicle_max_steering_angle_deg,
        params.vehicle_sigma_lateral_velocity, params.vehicle_sigma_vertical_velocity, params.vehicle_sigma_roll_pitch_rate,
        params.vehicle_sigma_yaw_rate_bound, params.vehicle_min_forward_speed, params.vehicle_yaw_rate_bound_margin,
        params.vehicle_use_yaw_rate_bound, params.vehicle_chi2_multiplier, params.vehicle_max_lateral_velocity_residual,
        params.vehicle_max_vertical_velocity_residual, params.vehicle_max_roll_pitch_rate_residual,
        params.vehicle_max_yaw_rate_bound_residual, params.vehicle_gate_log, params.vehicle_gate_log_path);
  }
}
...
void VioManager::feed_measurement_imu(const ov_core::ImuData &message) {
  ...
  // Push back to the vehicle dynamics updater if it is enabled
  if (is_initialized_vio && updaterVehicleDynamics != nullptr) {
    updaterVehicleDynamics->feed_imu(message, oldest_time);
  }
}
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
  ...
  // Apply soft ground-vehicle dynamics constraints after propagation, before feature updates.
  if (updaterVehicleDynamics != nullptr) {
    bool did_vehicle_dynamics_update = updaterVehicleDynamics->update(state);
    updaterVehicleDynamics->clean_old_imu_measurements(message.timestamp + state->_calib_dt_CAMtoIMU->value()(0) - 0.10);
    if (did_vehicle_dynamics_update) {
      propagator->invalidate_cache();
    }
  }
