...
struct VioManagerOptions {
  ...
  /// If we should try to use soft ground-vehicle dynamics constraints
  bool try_vehicle_dynamics = false;

  /// Vehicle wheelbase in meters
  double vehicle_wheelbase = 1.66;

  /// Maximum front wheel steering angle in degrees
  double vehicle_max_steering_angle_deg = 30.0;

  /// JPL quaternion from IMU frame to vehicle dynamics frame [x,y,z,w]
  std::vector<double> vehicle_q_imu_to_vehicle = {0.0, 0.0, 0.0, 1.0};

  /// Soft lateral velocity pseudo-measurement noise in m/s
  double vehicle_sigma_lateral_velocity = 0.20;

  /// Soft vertical velocity pseudo-measurement noise in m/s
  double vehicle_sigma_vertical_velocity = 0.20;

  /// Soft roll/pitch angular-rate pseudo-measurement noise in rad/s
  double vehicle_sigma_roll_pitch_rate = 0.10;

  /// Soft Ackermann yaw-rate bound pseudo-measurement noise in rad/s
  double vehicle_sigma_yaw_rate_bound = 0.20;

  /// Minimum forward speed before the yaw-rate bound is applied
  double vehicle_min_forward_speed = 0.20;

  /// Extra allowed yaw-rate above the Ackermann bound in rad/s
  double vehicle_yaw_rate_bound_margin = 0.20;

  /// If we should softly enforce the Ackermann yaw-rate bound
  bool vehicle_use_yaw_rate_bound = true;

  /// Innovation gate multiplier for vehicle-dynamics pseudo-measurements
  double vehicle_chi2_multiplier = 5.0;

  /// Absolute residual sanity limits for vehicle-dynamics updates
  double vehicle_max_lateral_velocity_residual = 5.0;
  double vehicle_max_vertical_velocity_residual = 5.0;
  double vehicle_max_roll_pitch_rate_residual = 2.0;
  double vehicle_max_yaw_rate_bound_residual = 3.0;

  /// Optional CSV log for vehicle-dynamics gate decisions
  bool vehicle_gate_log = false;
  std::string vehicle_gate_log_path = "/tmp/openvins_vehicle_dynamics_gate.csv";
  ...
  void print_and_load_estimator(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("try_vehicle_dynamics", try_vehicle_dynamics, false);
      parser->parse_config("vehicle_wheelbase", vehicle_wheelbase, false);
      parser->parse_config("vehicle_max_steering_angle_deg", vehicle_max_steering_angle_deg, false);
      parser->parse_config("vehicle_q_imu_to_vehicle", vehicle_q_imu_to_vehicle, false);
      parser->parse_config("vehicle_sigma_lateral_velocity", vehicle_sigma_lateral_velocity, false);
      parser->parse_config("vehicle_sigma_vertical_velocity", vehicle_sigma_vertical_velocity, false);
      parser->parse_config("vehicle_sigma_roll_pitch_rate", vehicle_sigma_roll_pitch_rate, false);
      parser->parse_config("vehicle_sigma_yaw_rate_bound", vehicle_sigma_yaw_rate_bound, false);
      parser->parse_config("vehicle_min_forward_speed", vehicle_min_forward_speed, false);
      parser->parse_config("vehicle_yaw_rate_bound_margin", vehicle_yaw_rate_bound_margin, false);
      parser->parse_config("vehicle_use_yaw_rate_bound", vehicle_use_yaw_rate_bound, false);
      parser->parse_config("vehicle_chi2_multiplier", vehicle_chi2_multiplier, false);
      parser->parse_config("vehicle_max_lateral_velocity_residual", vehicle_max_lateral_velocity_residual, false);
      parser->parse_config("vehicle_max_vertical_velocity_residual", vehicle_max_vertical_velocity_residual, false);
      parser->parse_config("vehicle_max_roll_pitch_rate_residual", vehicle_max_roll_pitch_rate_residual, false);
      parser->parse_config("vehicle_max_yaw_rate_bound_residual", vehicle_max_yaw_rate_bound_residual, false);
      parser->parse_config("vehicle_gate_log", vehicle_gate_log, false);
      parser->parse_config("vehicle_gate_log_path", vehicle_gate_log_path, false);
    ...
    if (vehicle_q_imu_to_vehicle.size() != 4) {
      PRINT_WARNING(YELLOW "vehicle_q_imu_to_vehicle should have 4 entries [x,y,z,w], using identity!\n" RESET);
      vehicle_q_imu_to_vehicle = {0.0, 0.0, 0.0, 1.0};
    }
    ...
    PRINT_DEBUG("  - vehicle_dynamics_update: %d\n", try_vehicle_dynamics);
    PRINT_DEBUG("  - vehicle_wheelbase: %.3f\n", vehicle_wheelbase);
    PRINT_DEBUG("  - vehicle_max_steering_angle_deg: %.2f\n", vehicle_max_steering_angle_deg);
    PRINT_DEBUG("  - vehicle_q_imu_to_vehicle: %.4f, %.4f, %.4f, %.4f\n", vehicle_q_imu_to_vehicle.at(0),
                vehicle_q_imu_to_vehicle.at(1), vehicle_q_imu_to_vehicle.at(2), vehicle_q_imu_to_vehicle.at(3));
    PRINT_DEBUG("  - vehicle_sigma_lateral_velocity: %.3f\n", vehicle_sigma_lateral_velocity);
    PRINT_DEBUG("  - vehicle_sigma_vertical_velocity: %.3f\n", vehicle_sigma_vertical_velocity);
    PRINT_DEBUG("  - vehicle_sigma_roll_pitch_rate: %.3f\n", vehicle_sigma_roll_pitch_rate);
    PRINT_DEBUG("  - vehicle_sigma_yaw_rate_bound: %.3f\n", vehicle_sigma_yaw_rate_bound);
    PRINT_DEBUG("  - vehicle_min_forward_speed: %.3f\n", vehicle_min_forward_speed);
    PRINT_DEBUG("  - vehicle_yaw_rate_bound_margin: %.3f\n", vehicle_yaw_rate_bound_margin);
    PRINT_DEBUG("  - vehicle_use_yaw_rate_bound?: %d\n", vehicle_use_yaw_rate_bound);
    PRINT_DEBUG("  - vehicle_chi2_multiplier: %.3f\n", vehicle_chi2_multiplier);
    PRINT_DEBUG("  - vehicle_max_lateral_velocity_residual: %.3f\n", vehicle_max_lateral_velocity_residual);
    PRINT_DEBUG("  - vehicle_max_vertical_velocity_residual: %.3f\n", vehicle_max_vertical_velocity_residual);
    PRINT_DEBUG("  - vehicle_max_roll_pitch_rate_residual: %.3f\n", vehicle_max_roll_pitch_rate_residual);
    PRINT_DEBUG("  - vehicle_max_yaw_rate_bound_residual: %.3f\n", vehicle_max_yaw_rate_bound_residual);
    PRINT_DEBUG("  - vehicle_gate_log?: %d\n", vehicle_gate_log);
    PRINT_DEBUG("  - vehicle_gate_log_path: %s\n", vehicle_gate_log_path.c_str());
