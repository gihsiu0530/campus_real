...
Eigen::VectorXd MPCPlanner_path::polyfit(const Eigen::VectorXd& xvals,const Eigen::VectorXd& yvals,int order)
{
    ...
    std::cout<<"xvalsize = "<< xvals.size()<<std::endl;
...
double MPCPlanner_path::anglarRegularization(nav_msgs::Odometry& base_odometry,double delta_d)
{
    ...
    void MPCPlanner_path::initialize()
    {
        ...
        private_nh_.param<std::string>("save_filename", filename_, ros::package::getPath("mpc_4state") + "/../../mpcdata/test.csv");
    ...
    void MPCPlanner_path::computelocalpath(const nav_msgs::OdometryConstPtr &msg)
    {
       ...
       // next_py = py + vt*sin(theta1);
        ...
        {
	    start_id = -1; 
	    ROS_WARN("Nearest distance exceeds threshold. Resetting start_id to 0.");
	}
	...
	int valid_front = std::min(front_waypoint_num_, static_cast<int>(global_path_x.size() - start_id));
	for (int i = 0; i < valid_front; i++) 
	{
	    org_wp_rearange_waypoint_x.push_back(global_path_x[start_id + i]);
	    org_wp_rearange_waypoint_y.push_back(global_path_y[start_id + i]);
	}

	int valid_back = std::min(back_waypoint_num_, start_id);
	for (int i = 0; i < valid_back; i++)
	{
	    org_wp_rearange_waypoint_x.push_back(global_path_x[start_id - i]);
	    org_wp_rearange_waypoint_y.push_back(global_path_y[start_id - i]);
	}

      	std::cout<<"org_wp_rearange_waypoint_x = "<<org_wp_rearange_waypoint_x.size()<<std::endl;
      	      	std::cout<<"org_wp_rearange_waypoint_x = "<<org_wp_rearange_waypoint_x.size()<<std::endl;
	...
	//double theta_trj1 = atan2(y_local,x_local);
        ...
        Eigen::Vector2d u_r(desired_speed,regularizeAngle(delta_d));
...
Eigen::Vector2d MPCPlanner_path::_mpcControl(Eigen::Vector3d s,Eigen::VectorXd s_d,Eigen::Vector2d u_r,Eigen::Vector2d du_p)
{
    ...
    Eigen::Vector2d du_min(-0.05,-0.05);

    //宣告一個2*1向量為 du_max 控制輸入變化量最大值
    //第一項為速度變化量 第二項為角速度變化量
    Eigen::Vector2d du_max(0.05,0.05);
    ...
    lower.topRows(dim_u * m_) = U_min - U_k_1 - U_r - Eigen::VectorXd::Constant(U_min.size(), epsilon);
    ...
    upper.topRows(dim_u * m_) = U_max - U_k_1 -U_r + Eigen::VectorXd::Constant(U_max.size(), epsilon);
