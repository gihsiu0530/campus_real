...
void MPCPlanner_path::computelocalpath(const nav_msgs::OdometryConstPtr &msg)
{
    ...
    if (global_path_x.size()!=0)
    {
        ...
        size_t window = 10000; // 只往前看最多 50 個點，可依路徑密度調整
        ...
        // === 切換時鎖定到轉折點 ===
        if (proj_idx >= turn_index_ && !reversed_mode) 
        {
            //reversed_mode = true;
        ...
        //double epsi = regularizeAngle(theta - (refHeading + (reversed_mode ? M_PI : 0.0)));
        ...
        // if (!reversed_mode)
        // {
        //     // // 2) 視窗改成左右對稱 + 高斯加權（非常小改，減少相位延遲）
        //     int H = 3;                // 原本等效 M=4 -> H=2 (範圍 -2..+2 共 5 點，也可改 H=1 對應 3 點)
        //     double sigma_idx = 1.0;   // 高斯權重在索引上的 σ

        //     double wsum = 0.0, ksum = 0.0;
        //     const int N = (int)global_path_x.size();
        //     for (int dj = -H; dj <= H; ++dj)
        //     {
        //         int ii = nearestIndex + dj;
        //         if (ii < 0)       ii = 0;
        //         if (ii > N - 1)   ii = N - 1;

        //         double w = std::exp(-0.5 * (dj * dj) / (sigma_idx * sigma_idx));
        //         double k = computeKappaAt(ii);
        //         ksum += w * k;
        //         wsum += w;
        //     }
        //     kappa_avg = (wsum > 1e-9) ? (ksum / wsum) : 0.0;

        //     static bool k_inited = false;
        //     static double kappa_filt = 0.0;
        //     double beta = 0.85;
        //     if (!k_inited) { kappa_filt = kappa_avg; k_inited = true; }
        //     else           { kappa_filt = beta * kappa_filt + (1.0 - beta) * kappa_avg; }
        // }
        // else
        // {
        ...
        // // 2) 視窗改成左右對稱 + 高斯加權（非常小改，減少相位延遲）
        // int H = 4;                // 原本等效 M=4 -> H=2 (範圍 -2..+2 共 5 點，也可改 H=1 對應 3 點)
        // double sigma_idx = 1.0;   // 高斯權重在索引上的 σ
        ...
        //     wsum += w;
        // }
        // double kappa_avg = (wsum > 1e-9) ? (ksum / wsum) : 0.0;

        // 3) （可選）跨時間一階低通，進一步去抖（保留反應性，值越大越平）
        // static bool k_inited = false;
        // static double kappa_filt = 0.0;
        // double beta = 0.85;
        // if (!k_inited) { kappa_filt = kappa_avg; k_inited = true; }
        ...
        double dx = px - global_path_x[turnIdx];
        double dy = py - global_path_y[turnIdx];
        //double dist_to_turn = std::sqrt(dx*dx + dy*dy);
        ...
        if (reversed_mode) {
            double mag = std::max(std::abs(v_ref), v_min_ref); // 至少保有最小速度幅度
