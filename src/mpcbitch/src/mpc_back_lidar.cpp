...
void MPCPlanner_path::initialize()
{
    ...
    if(!initialize_ )
    {
        ...
        vreal_pub = nh_.advertise<std_msgs::Float64>("/v_real",1000);
...
void MPCPlanner_path::computelocalpath(const nav_msgs::OdometryConstPtr &msg)
{
    ...
    double yaw_raw = tf2::getYaw(msg->pose.pose.orientation);   // odom 給你的真正朝向
    double t_now   = msg->header.stamp.toSec();

    // 記住上一筆 odom，拿來做差
    static bool   has_last_odom = false;
    ...
    if (has_last_odom) {
        ...
        if (dt > 1e-4) {
            double dx = px_raw - last_x;
            double dy = py_raw - last_y;

            // 你的程式後面有把 yaw 減去 1.571，所以這邊也用同一個座標方向來投影
            double yaw_for_body = yaw_raw - 1.571;

            // 投影到車頭方向，得到有正負的車速
            double v_from_odom = (dx * std::cos(yaw_for_body) + dy * std::sin(yaw_for_body)) / dt;

            // 簡單防呆：不要讓一筆跳太誇張
            if (std::isfinite(v_from_odom) && std::fabs(v_from_odom) < 1.0) 
            {
                // 直接覆寫你原本的 v_real，後面程式就都用這個
                v_real = v_from_odom;
            }
        }
    }
    ...
    if (global_path_x.size()!=0)
    {
        ...
        if (reversed_mode && vel_state != DWELL) 
        {
            double mag;
