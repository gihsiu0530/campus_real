...
static bool second_forward_mode = false;
...
void MPCPlanner_path::computelocalpath(const nav_msgs::OdometryConstPtr &msg)
{
    ...
    if (global_path_x.size()!=0)
    {
        ...
        // 預設先給一個尾端保護數量，等一下第三段要用
        int tail_guard = 5;   // 倒退到最後 5 個點就可以準備切回前進
        
        if (second_forward_mode) {
            // 第二次前進：鎖定在路徑尾端這一小段
            i_begin = (Y > (size_t)tail_guard) ? (Y - (size_t)tail_guard) : 0;
            i_end   = Y - 1;    // 掃到最後
        }
        else if (vt > V_EPS) 
        {
            // 第一次前進：只找 turn_index_ 之前
            i_begin = 0;
            i_end   = (turn_index_ > 0) ? (size_t)turn_index_ : 0;
        } 
        else if (vt < -V_EPS) 
        {
            // 倒退：只找 turn_index_ 之後
        ...
        if (reversed_mode && v_body < 0 && !second_forward_mode)
        {
            pi_offset = M_PI;
        }
        ...
        bool forward_like = (!reversed_mode) || second_forward_mode;
        double v_min_ref = forward_like ? min_v_forward_ : min_v_reverse_;
        double v_max_ref = forward_like ? max_v_forward_ : max_v_reverse_;
        ...
        // }
        // kappa_avg = kappa_sum / double(M);

        int M = 10; //4  mirror_back4  //6 mirror_back3  //3 all the other
        double kappa_sum = 0.0;
        for (int j = 0; j < M; ++j) 
        {
        ...
        // 觸發倒退（跟你原本一樣的條件，再多一個限制：不能已經進第二次前進）
        if (!reversed_mode && !second_forward_mode && py < global_path_y[turnIdx]) 
        {
            reversed_mode = true;
            ROS_INFO(">>> ENTER REVERSE MODE <<<");
        }

        // 倒退到快尾端時，切回第二次前進
        // 用「索引已經走到倒退段的最後幾個點」來判斷，比較不會受 y 正負跳影響
        int tail_guard2 = 5;   // 看你要倒退到倒數幾個點再往前
        if (reversed_mode && !second_forward_mode) {
            if (nearestIndex >= (int)global_path_x.size() - tail_guard2) {
                reversed_mode = false;
                second_forward_mode = true;
                // 速度重置一下，避免馬上用到倒車的負速度
                prev_v_ref = 0.0;
                ROS_INFO(">>> ENTER 2ND FORWARD MODE <<<");
            }
        }
        ...
        double delta_d = sign_v * std::atan(kappa_avg * L);
