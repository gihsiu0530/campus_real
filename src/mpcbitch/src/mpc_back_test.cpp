...
double MPCPlanner_path::dist(const geometry_msgs::PoseStamped &node1,
  ...
  return std::hypot(node1.pose.position.x - node2.pose.position.x,
                    node1.pose.position.y - node2.pose.position.y);
}

// std::floor 為向下取整數
//  angle + M_PI 將原始角度angle+3.14 使原本的角度變成0～6.28
// 然後在除以2*3.14將原始角度映射成相應的整數值
// 這個整數值表示原始角度在2*3.14的區段中的位置 將上一步的整數值*2*3.14
// 得到原始角度在相應區段中的起始值 將原始角度映射到[0~2*3.14]
...
void MPCPlanner_path::initialize() {
  ...
  if (!initialize_) {
    ...
    mpc_result_pub =
        nh_.advertise<std_msgs::Float64MultiArray>("/mpc_result_test", 1000);
    ...
    carla_control_pub = nh_.advertise<carla_msgs::CarlaEgoVehicleControl>(
        "/carla/ego_vehicle/vehicle_control_cmd", 1);

    // hybrid_path_sub = nh_.subscribe<nav_msgs::Path>("/sPath",
    // 1000,&MPCPlanner_path::hybridPathCallback, this);
  ...
  if (global_path_x.size() != 0) {
    ...
    if (!reversed_mode) {
      ...
      if (begin < cap)
        begin = cap;
      if (end < begin)
        end = std::min(begin + WINDOW, global_path_x.size());
    }
    ...
    double sign = (refDir.x() * errorVec.y() - refDir.y() * errorVec.x()) >= 0
                      ? 1.0
                      : -1.0;
    cte *= sign;
    ...
    // // // 限制範圍 (Clamp)
    look_ahead_dist = std::max((int)min_lookahead,
    ...
    case DWELL:
      ...
      break;
    case RESUME:
      if (std::abs(prev_v_ref) >= v_min_ref + 0.1 * (v_max_ref - v_min_ref))
        vel_state = CRUISE;
      break;
    }
    ...
    // 4. 曲率/CTE 基本降速
    // double curvature_threshold = 0.035;  //0.03 for 0.2m間隔 //0.02 for
    // 0.5m間隔 //曲率門檻
    ...
    // 4. 計算速度比例 (0.0 ~ 1.0)
    double speed_ratio =
        (current_v - v_slow_bound) / (v_fast_bound - v_slow_bound);
    ...
    if (kappa_for_speed > k_min_dynamic) {
      // 計算減速比例 f (0.0 ~ 1.0)
      double f =
          (kappa_for_speed - k_min_dynamic) / (k_max_dynamic - k_min_dynamic);
    ...
    //     if(!enddd)
    //     {
    //         mag = std::max(std::abs(v_ref), v_min_ref); //
    //         至少保有最小速度幅度
    //     }
    ...
    //         //double scale = std::cbrt(static_cast<double>(remain_pts) /
    //         DECEL_POINTS_END); double scale =
    //         std::pow(static_cast<double>(remain_pts) / DECEL_POINTS_END,
    //         0.25);
    //         // 6. 線性插值計算新速度
    ...
    const double CTE_ENTER = 0.02;
    const double EPSI_ENTER = 0.005; // 0.57度

    const double CTE_EXIT = 0.035;   // 3.5 cm
    const double EPSI_EXIT = 0.025; // 1.43度

    const int DEBOUNCE_CYCLES = 3; // 連續 N 幀滿足
    ...
    // std::cout<<" 控制週期 = "<< caculate_mpc_start -
    // caculate_mpc_finish<<std::endl;
...
Eigen::Vector2d MPCPlanner_path::_mpcControl(Eigen::Vector4d s,
                                             Eigen::Vector4d s_d,
                                             Eigen::Vector2d u_r,
                                             Eigen::Vector2d du_p) {
  int dim_u = 2;
  ...
  B_o(2, 1) = d_t_ * s_d(3) /
              (L * pow(cos(u_r[1]), 2)); // 轉向角 delta 影響航向角 theta
  ...
  // 優化問題設定
  Eigen::VectorXd Yr = Eigen::VectorXd::Zero(
      dim_x * p_); // 目標參考軌跡設為0 (因為我們追蹤的是誤差)
  ...
  // 2. 檢查是否處於倒車模式 (假設 reversed_mode 是 MPCPlanner_path
  // 類別的成員變數)
  if (reversed_mode) {
  ...
  Eigen::MatrixXd Q =
      Eigen::kroneckerProduct(Eigen::MatrixXd::Identity(p_, p_), Q_dynamic);
  Eigen::MatrixXd R =
      Eigen::kroneckerProduct(Eigen::MatrixXd::Identity(m_, m_), R_dynamic);
  ...
  Eigen::VectorXd U_min =
      Eigen::kroneckerProduct(Eigen::VectorXd::Ones(m_), u_min);
  Eigen::VectorXd U_max =
      Eigen::kroneckerProduct(Eigen::VectorXd::Ones(m_), u_max);
  Eigen::VectorXd dU_min =
      Eigen::kroneckerProduct(Eigen::VectorXd::Ones(m_), du_min);
  Eigen::VectorXd dU_max =
      Eigen::kroneckerProduct(Eigen::VectorXd::Ones(m_), du_max);
  Eigen::VectorXd U_r = Eigen::kroneckerProduct(Eigen::VectorXd::Ones(m_), u_r);
  Eigen::VectorXd U_k_1 =
      Eigen::kroneckerProduct(Eigen::VectorXd::Ones(m_), u_k_1);

  // 建立約束矩陣 P
  Eigen::MatrixXd temp =
      Eigen::MatrixXd::Ones(m_, m_).triangularView<Eigen::Lower>();
  Eigen::MatrixXd A_I =
      Eigen::kroneckerProduct(temp, Eigen::MatrixXd::Identity(dim_u, dim_u));
  ...
  Eigen::Vector2d u(solution[0] + du_p[0] + u_r[0],
                    regularizeAngle(solution[1] + du_p[1] + u_r[1]));
  u_prev = u;
...
MPCPlanner_path::_convertTOSparseMatrix(Eigen::MatrixXd A) {
  int row = A.rows();
  int col = A.cols();
