...
double MPCPlanner_path::anglarRegularization(nav_msgs::Odometry& base_odometry,double delta_d)
{
    ...
    void MPCPlanner_path::initialize()
    {
        ...
        private_nh_.param<std::string>("save_filename", filename_, ros::package::getPath("mpcbitch") + "/../../mpcdata/simulation.csv");
    ...
    void MPCPlanner_path::computelocalpath(const nav_msgs::OdometryConstPtr &msg)
    {
        ...
        //double norm_theta = regularizeAngle(theta1);
        //theta = unwrapAngle(norm_theta, prev_theta);
        //prev_theta = theta;
        ...
        if (start_id > global_path_x.size() - 10) 
        {
            ROS_INFO("finish");
       ...
       // 1. 車輛全局位置
        ...
        /*
        else if (fabs(cte) < 0.1)
        {
            cte = 0;
        }

        // 4. 計算航向誤差 epsi（車輛朝向 theta 與參考路徑切線角的差值）
        ...
        */
        ...
        //Eigen::Vector2d p0;
        //if (nearestIndex <= 0) {
            //p0 = Eigen::Vector2d(global_path_x[0], global_path_y[0]);
       // } else {
           // p0 = Eigen::Vector2d(global_path_x[nearestIndex - 1], global_path_y[nearestIndex - 1]);
       // }
        /*
        Eigen::Vector2d p0, p1;
        if (nearestIndex <= 0) 
        {
           // 當 nearestIndex 不合理時，使用第一和第二個點來計算路徑方向
           p0 = Eigen::Vector2d(global_path_x[0], global_path_y[0]);
           // 確保路徑至少有兩個點
           if (global_path_x.size() > 1) {
               p1 = Eigen::Vector2d(global_path_x[1], global_path_y[1]);
           } else {
               p1 = p0;  // 或者採取其他適當的處理
           }
        } 
        ...
        {
           p0 = Eigen::Vector2d(global_path_x[nearestIndex - 1], global_path_y[nearestIndex - 1]);
           p1 = Eigen::Vector2d(global_path_x[nearestIndex],   global_path_y[nearestIndex]);
        }
        */
        ...
        //Eigen::Vector2d p2(global_path_x[nearestIndex+1], global_path_y[nearestIndex+1]);

        /*
        for (int i = nearestIndex + 1; i <= nearestIndex + 10; ++i) 
        {
        ...
        */
        ...
        // 以 x = 0（即 projection 點對應的位置）計算導數，並求得曲率
        double dx0 = polyDerivative(coeffs, 0.0);
        double ddx0 = polySecondDerivative(coeffs, 0.0);
        double polyKappa = fabs(ddx0) / pow(1 + dx0 * dx0, 1.5);
        ...
        /*
        static int consecutive_cte_count = 0;
        double cte_threshold = 0.1;         // 設定門檻
        double curvature_threshold = 0.05;    // 曲率門檻

        // 基礎目標速度
        double v_max_ref = max_v_;  // 最大目標速度
        double v_min_ref = min_v_;  // 最小目標速度

        // 初始化速度參考為最小速度
        double v_ref = v_min_ref;
        
        // 如果車輛已經超過前 10 個路徑點，才考慮動態調整
        if (start_id >= 10) {
            // 使用靜態變數或類成員來計算連續滿足條件的次數
            static int consecutive_cte_count = 0;

            // 判斷當前 cte 是否小於門檻
            if (fabs(cte) < cte_threshold) {
                consecutive_cte_count++;   // 累加連續計數器
            } else {
                consecutive_cte_count = 0;   // 若不滿足則重置計數器
            }

            // 當連續 5 次滿足條件時，根據 cte 與曲率計算調整因子
            if (consecutive_cte_count >= 5) {
                double error_factor = std::min(1.0, std::max(0.0, 1.0 - (fabs(cte) / cte_threshold + fabs(kappa) / curvature_threshold)));
                v_ref = v_min_ref + error_factor * (v_max_ref - v_min_ref);
            }
        } 
        else 
        {
            // 如果處於路徑前 10 個點，直接使用最低速度（不做調整）
            v_ref = vt;
        }
        */
        /*
        // 參數設定
        double v_max_ref = max_v_;         // 直線最大目標速度
        double v_min_ref = min_v_;         // 最低目標速度
        double curvature_threshold = 0.0003; // 曲率門檻
        double cte_threshold = 0.08;        // cte 門檻
        double cte_scale = 0.2;            // cte 調整尺度
        int lookahead_points = 10;          // 增加預知點數
        int stable_window = 3;             // 滑動窗口大小，連續 5 次穩定
        
        // 假設有一個全局或類成員的滑動窗口保存最近 N 次 cte 值
        ...
        if (start_id < 5) 
        {
            double ratio = static_cast<double>(start_id) / 10.0;
            v_ref = vt * (1 - ratio) + v_min_ref * ratio;  // 平滑过渡
        }
        else if (start_id >= (global_path_x.size() - 15)) {
            // 最后15个点平滑减速到最低速
            double ratio_to_min_speed = static_cast<double>(global_path_x.size() - start_id) / 15.0;
            v_ref = v_min_ref + ratio_to_min_speed * (v_max_ref - v_min_ref);
        }
        else {
            // 更新 cte 滑动窗口
            ...
            if (avg_cte < cte_threshold) {
                ...
                if (predicted_curvature > curvature_threshold) {
                    // 增加曲率的衰减因子，进一步减速
                    factor_curvature = std::max(0.0, 1.0 - (predicted_curvature - curvature_threshold) * 2);  // 更强的减速响应
                }
                ...
                // 插值计算速度参考
                double new_v_ref = v_min_ref + combined_factor * (v_max_ref - v_min_ref);
        
                // 让 v_ref 一次最多只能改变 0.03
                double max_delta_v_ref = 0.05;
                v_ref = std::clamp(new_v_ref, v_ref - max_delta_v_ref, v_ref + max_delta_v_ref);
            } 
            else 
            {
                // 如果平均 cte 太大，保持低速
        ...
        */
        ...
        if (start_id < 5) {
            ...
            double ratio_to_min_speed = static_cast<double>(global_path_x.size() - start_id) / 15.0;
            desired_v_ref = v_min_ref + ratio_to_min_speed * (v_max_ref - v_min_ref);
        }
        else {
            ...
            if (avg_cte < cte_threshold) {
                double factor_curvature = 1.0;
                if (predicted_curvature > curvature_threshold) {
                    factor_curvature = std::max(0.0, 1.0 - (predicted_curvature - curvature_threshold) * 2);
                }
        ...
        double max_delta_v = 0.05; // 每次最大變化限制
        ...
        /*
        static bool in_straight_line = true;  // 标志是否处于直线路径
        static double stable_time = 0.0;      // 用于判断路径是否稳定为直线
        double kappa_threshold = 0.0002;     // 曲率小于此值时认为是直线
        double time_threshold = 0.6;          // 曲率稳定超过 2 秒时重新认为是直线

        // 判断是否处于直线路径，如果 kappa 变化过小且 cte 和 epsi 都足够小，就进入直线模式
        if (kappa <= kappa_threshold && last_kappa - kappa <= 0.0003) {
            // 如果曲率非常小且误差都很小，进入直线段
            if (fabs(cte) < 0.01 && fabs(epsi) < 0.01 && !in_straight_line) {
                // 判断当前曲率是否已经稳定一段时间
                stable_time += d_t_;  // 假设 d_t_ 是每个控制周期的时间间隔
                if (stable_time >= time_threshold) {
                    in_straight_line = true;  // 如果曲率长时间稳定，则认为已经回到直线
                }
            }
        } else 
        {
            // 一旦进入弯道或曲率大于阈值时，恢复到正常控制
            in_straight_line = false;
            stable_time = 0.0;  // 重置稳定时间
        }
        */
       ...
       // cte_pub.publish(cte_test);
	    ...
	    if (!header_written) {
		ofs << "u_v,v_ref,u_delta,delta_d,px,py,theta1,cte,cte_real,epsi,vx,vy,kappa" << std::endl;
		header_written = true;
	    }
